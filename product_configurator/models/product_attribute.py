...
class ProductAttribute(models.Model):
    ...
    def copy(self, default=None):
        ...
        It prevents attribute having same name while copying.
        """
        if not default:
            default = {}
    ...
    def validate_custom_val(self, val):
        ...
        if self.custom_type in ("integer", "float"):
            ...
            if minv and maxv and (val < minv or val > maxv):
                ...
                raise ValidationError(
                    _(
                        "Selected custom value '%(name)s' must be lower "
                        "than %(max_val)s",
                        name=self.name,
...
class ProductAttributeLine(models.Model):
    ...
    def onchange_attribute(self):
        """Set default value of required/multi/custom from attribute."""
    ...
    def _check_default_values(self):
        """Check if values are not outside of the values selected in attribute line."""
        for line in self.filtered(lambda ln: ln.default_val):
    ...
    def _check_valid_values(self):
        """Overwrite to save attribute line without values when custom is true."""
        for ptal in self:
            # Customization
...
class ProductAttributeValue(models.Model):
    ...
    def name_search(self, name="", args=None, operator="ilike", limit=100):
        """Use name_search as a domain restriction for the frontend.

        Show only values set on the product template taking all the configuration
