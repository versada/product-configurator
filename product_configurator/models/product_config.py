...
class ProductConfigDomain(models.Model):
    ...
    def _get_trans_implied(self):
        """Compute the transitive closure of relation implied_ids."""
    ...
    def compute_domain(self):
        ...
        Return a list of domains defined on a product.config.domain_line_ids
        and all implied_ids.
        """
...
class ProductConfigSession(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if product_tmpl:
                default_val_ids = (
                    product_tmpl.attribute_line_ids.filtered(lambda ln: ln.default_val)
                    .mapped("default_val")
                    .ids
                )
    ...
    def get_cfg_price(self, value_ids=None, custom_vals=None):
        ...
        It computes the price of the configured product based on the
        configuration passed in via value_ids and custom_values
        :param value_ids: list of attribute value_ids
        :param custom_vals: dictionary of custom attribute values
    ...
    def _get_config_image(self, value_ids=None, custom_vals=None, size=None):
        ...
        Retrieve the image object that most closely resembles the configuration
    ...
    def get_next_step(
        ...
        """Find and return next step if it exists.

        This usuallyimplies the next configuration step (if any) defined
        via the config_step_line_ids on the product.template.
        """
        if not product_tmpl_id:
            product_tmpl_id = self.product_tmpl_id
    ...
    def get_open_step_lines(self, value_ids=None):
        """Return a recordset of configuration step lines.

        Configuration step lines open for access given
    ...
    def get_all_step_lines(self, product_tmpl_id=None):
        """Return a recordset of configuration step lines of product_tmpl_id.
    ...
    def get_adjacent_steps(self, value_ids=None, active_step_line_id=None):
        """Return the previous and next steps given.

        The configuration passed via value_ids and the active
        step line passed via cfg_step_line_id.
        """
        ...
        active_cfg_step_line = config_step_lines.filtered(
            lambda ln: ln.id == active_step_line_id
        )
    ...
    def get_variant_search_domain(self, product_tmpl_id, value_ids=None):
        """Get variant search domain.

        This method called by search_variant used to search duplicates in the database.
        """
        if value_ids is None:
            value_ids = self.value_ids.ids
    ...
    def values_available(
        ...
        for attr_val_id in check_val_ids:
            config_lines = product_tmpl.config_line_ids.filtered(
                lambda ln: attr_val_id in ln.value_ids.ids
            )
    ...
    def get_extra_attribute_line_ids(self, product_template_id):
        """Retrieve attribute lines.

        Attribute lines defined on the product_template_id
    ...
    def validate_configuration(
        ...
        if not set(custom_vals.keys()) <= set(custom_attr_ids):
            ...
            for attr_id in custom_attrs_with_error:
                message_vals += "\n%s: %s", attr_id.name, custom_vals.get(attr_id.id)
    ...
    def search_variant(self, value_ids=None, product_tmpl_id=None):
        ...
        Searches with ith given value_ids and custom values given in the
        custom_vals dict.
        :param value_ids: list of product.attribute.values ids
        :param custom_vals: dict {product.attribute.id: custom_value}
    ...
    def encode_custom_values(self, custom_vals):
        """Encode to alter the values of the custom values before creating or writing.
    ...
    def get_child_specification(self, model, parent):
        """Return dict of onchange specification by appending parent before each key."""
