...
class ProductTemplate(models.Model):
    ...
    def _compute_product_variant_count(self):
        """Extend to for configurable products.

        Return the number of variants configured or 1 as
        many views and methods trigger only when a template
        has at least one variant attached. Since we create
        them from the template we should have access to them always.
        """
    ...
    def _compute_template_attr_vals(self):
        """Compute all attribute values added in attribute line on product template."""
        for product_tmpl in self:
    ...
    def check_attr_value_ids(self):
        ...
        Attribute lines should not have some attribute value that
    ...
    def _validate_unique_config(self):
        """Check for duplicate configurations in image lines."""
    ...
    def _create_variant_ids(self):
        """Prevent configurable products from creating variants.

        Variants serve only as a template for the product configurator.
        """
    ...
    def copy(self, default=None):
        """Copy restrictions, config Steps and attribute lines with product template."""
        if not default:
            default = {}
        self.ensure_one()
        self = self.with_context(check_constraint=False)
        res = super().copy(default=default)
        # In some cases, you get result with original product too.
        res = res if len(res) == 1 else res - self
        # Attribute lines
        attribute_line_dict = {}
    ...
    def configure_product(self):
        ...
        It launches a product configurator wizard with a linked
    ...
    def check_config_user_access(self):
        """Check access to perform action on configurable products."""
...
class ProductProduct(models.Model):
    ...
    def _get_mako_context(self, buf):
        """Return context needed for computing product name.

        It is based on mako-tamplate define on it's product template.
        """
        self.ensure_one()
    ...
    def _get_mako_tmpl_name(self):
        """Compute and return product name based on mako-template."""
        self.ensure_one()
    ...
    def _compute_config_name(self):
        """Get name of configurable product."""
        for product in self:
            if product.config_ok:
    ...
    def reconfigure_product(self):
        """Launch a product configurator wizard.

        Linked template and variant in order to re-configure an existing product.
        It is essentially a shortcut to pre-fill configuration data of a variant.
        """
        self.ensure_one()
